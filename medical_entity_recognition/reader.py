...
if __name__ == '__main__':
    ...
    #         fw.write('\n')
    # fw.close()
    basic_path = os.path.join(PROJECT_PATH, 'medical_entity_recognition')
    data_all = open(os.path.join(basic_path, 'data_char_all.txt'), 'w', encoding='utf-8')
    raw_datas = []
    with open(os.path.join(basic_path, 'train_test_data', 'train_bio_word.txt'),
    ...
    if len(data) != 0:
        raw_datas.append(data)
        data = ''
    with open(os.path.join(basic_path, 'train_test_data', 'test_bio_word.txt'),
              'r', encoding='utf-8') as fr:
        for line in fr:
            fields = line.strip().split('\t')
            if len(fields) == 3:
